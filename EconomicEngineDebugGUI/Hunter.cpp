...
Hunter::Hunter() : Job()
{
	...
	this->craftFactory->registerCraft(new Craft(0.5f, typeid(Meat).hash_code(), 1,std::vector<std::pair<size_t, int>>()));


	std::vector<size_t> toolsRequired;
	toolsRequired.emplace_back(typeid(BowBehavior).hash_code());
	this->craftFactory-> registerCraft(new Craft(0.5f, typeid(Leather).hash_code(), 1, std::vector<std::pair<size_t, int>>(), toolsRequired));
